...
const updateAS = ({ }) => {
    ...
    const item_fitns = item.fitnessInstructors;
    // console.log(item_fitns);                        // 傳進來的 item.fitn 為 Map(id, name)

    // 轉換邏輯：將 Map 的鍵值與容器 fitnessInstructors 匹配
    const matchedInstructors = Object.keys(item_fitns) // Map 的 key 列表
        .map((id) => fitnessInstructors.find((instructor) => instructor.id === Number(id)))
        .filter(Boolean); // 過濾掉找不到匹配對象的結果

    // console.log(matchedInstructors);
    ...
    const [formASInput, setFormASInput] = useState(     // From 表單
        {
            ...
            maxSignNumber: item.maxSignNumber,
            information: item.information,
            activityManager: item.activityManager,
            classRoom: item.classRoom,
            classType: item.classType,
            fitnessInstructors: matchedInstructors
        })
    ...
    const handleSubmit = async () => {
        ...
        // console.log("轉Map形式: ", formattedData);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <FormControl sx={{ minWidth: 200 }}>
                        ...
                        <Select
                            ...
                            label="課程時段"
                        >
                            <MenuItem value={formASInput.classTime}>
                                <em>{formASInput.classTime}</em>
                            </MenuItem>
                    ...
                    <FormControl fullWidth>
                        ...
                        >
                            <MenuItem value={formASInput.classType}>
                                <em>{formASInput.classType.name}</em>
                            </MenuItem>
                            {classTypes.map((type) => (
                    ...
                    <FormControl fullWidth>
                        ...
                        >
                            <MenuItem value={formASInput.classRoom}>
                                <em>{formASInput.classRoom.name}</em>
                            </MenuItem>
                            {classRooms.map((room) => (
